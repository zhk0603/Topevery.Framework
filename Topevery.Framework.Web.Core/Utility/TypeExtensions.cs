...
namespace Topevery.Framework.Utility
{
    /// <summary>
    ...
    public static class TypeExtensions
    {
        ...
        /// <returns> 是返回True，不是返回False </returns>项目。
        ...
        /// <returns></returns>
        public static List<KeyValuePair<string, int>> GetEnumItems(this Type type)
        {
            if (!type.IsEnum)
            ...
            {
                var tmp = new KeyValuePair<string, int>(Enum.GetName(type, item), (int) item);
                result.Add(tmp);
            }
            return result;
        }
